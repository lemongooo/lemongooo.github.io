...
```mermaid
graph TB
    A[测试用例层<br/>test_*.py] -->|调用| B[驱动层<br/>RequestBase]
    B -->|读取| C[数据层<br/>YAML配置]
    B -->|动态替换| D[辅助函数<br/>DebugTalk]
    B -->|发送请求| E[请求封装<br/>SendRequest]
    E -->|HTTP| F[服务器接口]
    F -->|响应| G[结果处理]
    G -->|参数提取| H[extract.yaml]
    G -->|断言验证| I[Assertions]
    I -->|生成| J[Allure报告]
    H -->|参数依赖| D
...
**设计思路说明:**
- **用例层**: 定义测试场景和执行流程,专注业务逻辑
- **驱动层**: 封装请求处理和断言逻辑,提升复用性
- **数据层**: YAML存储测试数据,实现用例与参数分离
...
```mermaid
graph LR
    A[接口响应] --> B{解析validation配置}
    B --> C[遍历断言列表]
    C --> D{断言类型?}
    
    D -->|contains| E[contains_assert]
    D -->|eq| F[equal_assert]
    D -->|ne| G[not_equal_assert]
    D -->|db| H[assert_mysql_data]
